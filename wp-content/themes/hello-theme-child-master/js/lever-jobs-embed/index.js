...
if(department != ""){
   ...
   var url = 'https://api.lever.co/v0/postings/' + options.accountName + '?group=team&mode=json&department='+ department +'';
   console.log(url);
...
for(var i = 0; i < groupedPostings.length; i++) {
  ...
  for (j = 0; j < groupedPostings[i].teams.length; j ++) {
    ...
    content += '<ul class="lever-team job-section" data-team="' + groupedPostings[i].teams[j].teamTitle + '"><li><h3 class="title">' + sanitizeForHTML(groupedPostings[i].teams[j].teamTitle) + '</h3><ul>';
    ...
    for (var k = 0; k < groupedPostings[i].teams[j].postings.length; k ++) {
      ...
      '<div class="posting-info"><h4><a class="lever-job-title text-green" target="_blank" href="' + groupedPostings[i].teams[j].postings[k].hostedUrl + '">' + sanitizeForHTML(groupedPostings[i].teams[j].postings[k].text) + '</a></h4><div class="posting-categories"><span class="lever-job-tag">' + (sanitizeForHTML(groupedPostings[i].teams[j].postings[k].categories.location) || '') + '</span><span class="lever-job-department">' + (sanitizeForHTML(groupedPostings[i].teams[j].postings[k].categories.department) || '') + ' - ' +  (sanitizeForHTML(groupedPostings[i].teams[j].postings[k].categories.team) || '') + '</span></div></div><div class="posting-apply"><a class="posting-btn-apply" target="_blank" href="' + groupedPostings[i].teams[j].postings[k].hostedUrl + '">Apply</a></div></li>';
    }
